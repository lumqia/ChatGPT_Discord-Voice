    ...
    if status is CompletionResult.OK or status is CompletionResult.MODERATION_FLAGGED:
        ...
        if not reply_text:
            ...
            for r in shorter_response:
                ...
                # C:/Users/Administrator/Desktop/ChadGPT/src/
                #channel = client.get_channel('1070069045652627486')
                # vc = await client.connect()
                engine.save_to_file(reply_text, 'speech.mp3')
                engine.runAndWait()
                # player = vc.create_ffmpeg_player('speech.mp3', after=lambda: print('done'))
                # player.start()
